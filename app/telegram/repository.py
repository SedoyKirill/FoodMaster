...
class BotRepository:
    ...
    async def notification_targets(self) -> list[dict[str, Any]]:
        """Кому вообще можно слать: привязанные и живые аккаунты.

        Отозванная привязка исчезает из ``auth_identities``, поэтому отдельного
        флага «не беспокоить после отвязки» не нужно — человек просто не
        попадает в выборку.
        """
        rows = await self.pool.fetch(
            """
            SELECT ai.provider_user_id::bigint AS telegram_id,
    ...
    async def notification_settings(self, telegram_id: int) -> dict[str, dict[str, Any]]:
        rows = await self.pool.fetch(
            """
            SELECT kind, enabled, hour, last_sent_on
            FROM app_core.telegram_notifications WHERE user_id=$1
            """,
            int(telegram_id),
        )
    ...
    async def set_notification(
        self, telegram_id: int, kind: str, enabled: bool, hour: int
    ) -> None:
        await self.pool.execute(
            """
            INSERT INTO app_core.telegram_notifications (user_id, kind, enabled, hour)
            VALUES ($1,$2,$3,$4)
            ON CONFLICT (user_id, kind) DO UPDATE
            SET enabled=EXCLUDED.enabled, hour=EXCLUDED.hour
            """,
            int(telegram_id), kind, bool(enabled), int(hour),
        )

    async def mark_notified(self, telegram_id: int, kind: str, day: Any) -> None:
        await self.pool.execute(
            """
            INSERT INTO app_core.telegram_notifications (user_id, kind, last_sent_on)
            VALUES ($1,$2,$3)
            ON CONFLICT (user_id, kind) DO UPDATE SET last_sent_on=EXCLUDED.last_sent_on
            """,
            int(telegram_id), kind, day,
        )

    async def expiring_lots(self, household_id: Any, until: Any) -> list[dict[str, Any]]:
        """Партии, которые испортятся не позже указанной даты."""
        rows = await self.pool.fetch(
            """
            SELECT name, quantity, unit_code, expires_on, storage_area
            FROM app_core.inventory_lots
    ...
    async def dishes_using(self, plan_id: Any, names: list[str]) -> dict[str, list[str]]:
        """Ингредиент → блюда плана, где он нужен.

        В ``plan_ingredients`` продукты уже сложены по всему плану, связи с
        конкретным блюдом там нет, поэтому идём через состав рецептов.
        """
        if not names:
            return {}
        rows = await self.pool.fetch(
            """
            SELECT DISTINCT ri.normalized_name AS name, r.title
            FROM app_core.plan_meals pm
            JOIN recipe_library.recipe_ingredients ri ON ri.recipe_id = pm.recipe_id
            JOIN recipe_library.recipes r ON r.id = pm.recipe_id
            WHERE pm.plan_id = $1 AND ri.normalized_name = ANY($2::text[])
            ORDER BY ri.normalized_name, r.title
            """,
            plan_id, [str(name) for name in names],
        )
        result: dict[str, list[str]] = {}
        for row in rows:
            result.setdefault(str(row["name"]), []).append(str(row["title"]))
        return result
